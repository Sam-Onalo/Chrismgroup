            ...
            setTimeout(function () {
                ...
                if (windowWidth > 991) {
                    $megaMenu.each(function () {
                        ...
                        $thisMegaMenu.css({'left': '50%', 'display': 'none', 'transform': 'translate(-50%)'});
        ...
        $('form.contact-form').on('submit', function (e) {
            ...
            var ajax = jQuery.post($form.attr('action'), request)
                .done(function (data) {
                    ...
                    $($form).find('[type="submit"]').attr('disabled', false).parent().append('<div class="contact-form-respond color-main mt-20" style="color: #053959 !important; word-break: break-word;">' + data.success + '</div>');
                    //cleaning form
                ...
                .fail(function (data) {
                    Notiflix.Loading.remove(600);
                    var errors = data.responseJSON.errors, msg = data.responseJSON.message + '<ul>';
                    for (const error in errors) {
                        if (Object.hasOwnProperty.call(errors, error)) {
                            msg += `<li> ${errors[error]} </li>`;
                        }
                    }
                    msg += '</ul>';
                    $($form).find('[type="submit"]').attr('disabled', false).blur().parent().append(`<div class="contact-form-respond color-main mt-20" style="word-break: break-word;">${msg}</div>`);
                })
        });
